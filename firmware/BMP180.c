...
bool BMP180_init(void) {
  ...
  // The BMP180 includes factory calibration data stored on the device. Each
  // device has different numbers, these must be retrieved and used in the
...
uint8_t BMP180_start_temperature(void) {
  ...
  // Return the delay in ms (rounded up) to wait before retrieving data, or
  // return 0 if there was a problem communicating with the BMP.
  return BMP180_write(data, 2) ? 5 : 0;
}

/*
 * Retrieve a previously-started temperature reading.
 * Requires BMP180_init() to be called once prior to retrieve calibration
 * parameters.
 * Requires BMP180_start_temperature() to have been called prior and sufficient
 * time elapsed.
...
/*
 * Retrieve a previously started pressure reading, calculate absolute pressure
 * in mbars.
 * Requires BMP180_init() to be called once prior to retrieve calibration
 * parameters.
 * Requires BMP180_start_pressure() to have been called prior and sufficient
 * time elapsed.
 ...
 * equivalent pressure (mb) at sea level.
